...
func (r *rpcRequest[Params, Resp]) Prepare(url string, opts ...PrepareOpt) *praparedRPCRequest[Resp] {
	...
	// Background here, not a caller context: Prepare has none. Callers attach
	// the real one with WithContext, which is applied below.
	req, err := http.NewRequestWithContext(context.Background(), http.MethodPost, url, buff)
	if err != nil {
